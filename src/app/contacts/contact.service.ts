...
export class ContactService {
  ...
  getContacts(): void {
    this.http.get(this.dbUrl + "/contacts").subscribe({
  ...
  deleteContact(contact: Contact) {
    ...
    this.http
      .delete<"application/json">(this.dbUrl + "/contacts/" + id)
      .subscribe({
        next: (responseData) => {
  ...
  addContact(newContact: Contact) {
    ...
    newContact.id = "1";
    this.http.post(this.dbUrl + "/contacts/1", newContact).subscribe({
  ...
  updateContact(newContact: Contact): Contact {
    ...
    // did we find it?
    if (i == this.contacts.length) return null;
    newContact.id = id;
    this.contacts[i] = newContact;
    this.http
      .put(this.dbUrl + "/contacts/" + newContact.id, newContact)
      .subscribe({
        next: (responseData) => {
          this.contactListChangedEvent.next(this.contacts.slice());
        },
