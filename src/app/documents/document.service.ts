...
export class DocumentService {
  ...
  deleteDocument(document: Document) {
    ...
    this.http.delete(this.dbUrl + "/documents/" + id).subscribe({
      next: (responseData) => {
  ...
  updateSubdocument(childIndex: number, parentDocument: Document) {
    ...
    this.http
      .post(this.dbUrl + "/subdocuments/" + childIndex, parentDocument)
      .subscribe({
  ...
  addDocument(newDocument: Document) {
    ...
    this.http.post(this.dbUrl + "/documents/1", newDocument).subscribe({
  ...
  updateDocument(newDocument: Document): Document {
    ...
    // did we find it?
    if (i == this.documents.length) return null;
    newDocument.id = id;
    this.documents[i] = newDocument;
    this.http
      .put<"application/json">(this.dbUrl + "/documents/" + id, newDocument)
      .subscribe({
        next: (responseData) => {
          this.documentListChangedEvent.next(this.documents.slice());
        },
