...
export class MessageService {
  ...
  getMessages(): void {
    ...
    this.http.get(this.dbUrl + "/messages").subscribe({
  ...
  deleteMessage(message: Message) {
    //may need more if user can separately delete a message.
    if (!message) {
      return;
    }
    const id = message.id;
    ...
    this.http
      .delete<"application/json">(this.dbUrl + "/messages" + id)
      .subscribe({
        next: (responseData) => {
  ...
  addMessage(newMessage: Message) {
    ...
    this.http.post(this.dbUrl + "/messages/0", newMessage).subscribe({
      next: (createdMessage: {
